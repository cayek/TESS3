least.square.solver <- function(A,b) {
  return(solve( crossprod(A,A) ,crossprod(A,b)))
}

Q.projection <- function(Q) {
...
X.bin <- function(X,d) {
  X.bin = matrix(0,nrow(X), ncol(X) * (d+1) )
  for(i in 1:nrow(X)) {
    for(l in 1:ncol(X)) {
      for(j in 0:d) {
        X.bin[i,1+(d+1)*(l-1) + j] = (X[i,l] == j)
      }
    }
  }
  return(X.bin)
...
#
#' @export
solverTESS3.projected <- function(X, K, d, Lapl, lambda, max.iteration) {
  ...
  for(it in 1:max.iteration) {
    ptm <- proc.time()
    # update G
    G = t(least.square.solver(Q,X))
    G = G.projection(G,L,D)
...
#
#' @export
solverTESS3.QP <- function(X, K, d, Lapl, lambda, max.iteration) {
