...
void sNMF(sNMF_param param)
{
	...
	if (strcmp(param->coord_input_file, "")) {
		...
		param->coordMatrix = (double *)calloc(n * 2, sizeof(double));
		read_data_double(param->coord_input_file, n, 2, param->coordMatrix);
		
		if (strcmp(param->input_file_W, "")) {
			
			// Read the W matrix and create L
			param->W = (double *)calloc(n * n, sizeof(double));
			param->Laplacian = (double *)calloc(n * n, sizeof(double));
			read_data_double(param->input_file_W, n, n, param->W);
			create_laplacian_matrix(param->W, param->Laplacian, n);			

		} else {
			// Create a L and W regularization matrix
			param->Laplacian = (double *)calloc(n * n, sizeof(double));
			param->W = (double *)calloc(n * n, sizeof(double));
			create_weight_laplacian_matrix(param->W, param->Laplacian, param->coordMatrix, n, param->neighborProportion);
			...
			strcpy(aux, tmp_file);
			strcat(aux, ".W");
			write_data_double(aux, param->n, param->n, param->W);
			printf("Write W matrix into file %s:"
				"\t\tOK.\n", aux);
			free(tmp_file);
			...
			strcpy(aux, tmp_file);
			strcat(aux, ".L");
			write_data_double(aux, param->n, param->n, param->Laplacian);
			printf("Write L matrix into file %s:"
				"\t\tOK.\n", aux);
			free(tmp_file);
		}

		//Normalize beta
		normalize_beta( &param->beta, param->n, param->Mc, param->W);
		printf( "Normalized beta parameter : %f \n", param->beta );
	...
	computeFst(param->Q, param->F, param->n, param->L, param->K, param->nc, param->Fst);
